---
id: 5
title: "Relational NFL"
subtitle: "A relational database for NFL statistics"
tags: ["Systems Programming"]
date: "2024-01-05T10:00:00"
services: ["Databases", "SQL", "Data Modeling"]
image: null
polished: false
link: "https://github.com/jackherberger/NFL_statistics"
---
A project to design and implement a relational database for storing and querying NFL statistics, with a focus on data modeling and efficient query design.